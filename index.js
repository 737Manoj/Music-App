...
function formatTime(seconds) {
  ...
  // Convert seconds to an integer to remove any milliseconds
...
async function getSongs(folder) {
  ...
  for (let index = 0; index < as.length; index++) {
   ...
   // show all songs in the playlist area of webpage
   ...
   Array.from(document.querySelector(".songList").getElementsByTagName("li")).forEach(e => {
     e.addEventListener("click", element => {
       //songInfo and songinfo are 2 diff divs only songInfo is created using js 
...
displayAlbums();
  ...
  // attach an evenListner to play, previous, and next
  ...
  document.querySelector(".range").getElementsByTagName("input")[0].addEventListener("change", (e)=>{
    console.log("Setting vol to",e.target.value)
